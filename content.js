...
function renderSettings() {
  ...
  setBody(`
    <div class="termscope-settings">
      ...
      <label class="termscope-field">AI backend endpoint<input id="ts-endpoint" value="${escapeHtml(settings.apiEndpoint || "")}" placeholder="http://localhost:8787/analyze"></label>
      <label class="termscope-field">Optional access token<input id="ts-token" type="password" value="${escapeHtml(settings.accessToken || "")}" placeholder="Leave blank for local backend"></label>
  ...
  host.querySelector("#ts-save").addEventListener("click", async () => {
    ...
    settings.apiEndpoint = host.querySelector("#ts-endpoint").value.trim();
    settings.accessToken = host.querySelector("#ts-token").value.trim();
    await chrome.storage.sync.set({ settings });
