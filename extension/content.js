const POLICY_TEXT = {
  terms:
    /\b(?:terms(?:\s+(?:of\s+use|of\s+service|and\s+conditions|&\s+conditions))?|conditions\s+of\s+use|user\s+agreement)\b/i,
...
const DEFAULT_SETTINGS = {
  ...
  apiEndpoint:
    "https://termscope-api.nsithamraju.workers.dev/analyze",
  widgetPosition: null
};
...
function chooseTriggerAnchor(items) {
  ...
  return (
    best?.[1]?.[best[1].length - 1]?.anchor ||
    items[items.length - 1].anchor
  );
}
...
function installSingleTrigger() {
  ...
  button.setAttribute(
    "aria-label",
    "Check the Terms and Privacy Policy"
  );
...
function createWidget() {
  ...
  host.innerHTML = `
    <div class="termscope-shell">
      <div
        class="termscope-header"
        id="termscope-drag-handle"
      >
        <div class="termscope-logo">
          ${shieldSvg}
        </div>
...
function applySavedPosition() {
  if (!host || !settings.widgetPosition) {
    return;
  }
...
function policyChoiceLabel(item) {
  let hostname = "";
  ...
  if (!choices.length) {
    setBody(`
      <div class="termscope-error">
        I could not find a Terms of Use or Privacy Policy link on this page.
      </div>
    `);
  ...
  setBody(`
    <div class="termscope-summary-head">
      <div>
        ...
        <p class="termscope-overview">
          Choose one policy for the fastest result, or check every policy found on this page.
        </p>
      </div>
    </div>
...
function extractRelevantText(
  ...
  if (
    output.length <
    Math.min(5000, maxChars)
  ) {
    output = cleaned.slice(
      0,
      maxChars
    );
  }
...
function htmlToReadableDocument(
  html,
  url,
  contentType
) {
...
async function analyzePolicies(
    ...
    <div class="termscope-status">
      ...
      <strong>
        Reading ${
          selected.length === 1
            ? "the policy"
            : `${selected.length} policies`
        }
      </strong>

      <p>
        TermScope is downloading the selected documents without leaving this page.
      </p>
    </div>
  `);
  ...
  try {
        ...
        selected.map(
          async (policy) => {
            ...
            if (!fetched?.ok) {
              throw new Error(
                fetched?.error ||
                `Could not read ${policy.label}`
              );
            }

            const document =
              htmlToReadableDocument(
                fetched.html,
                fetched.url,
                fetched.contentType
              );
            ...
            if (
              ...
              throw new Error(
                `${policy.label} did not contain enough readable text.`
              );
            }
    ...
    if (!documents.length) {
      ...
      throw new Error(
        errors[0] ||
        "None of the selected policies could be read."
      );
    }
    ...
    setBody(`
      <div class="termscope-status">
        ...
        <strong>
          Finding the important problems
        </strong>

        <p>
          The faster AI model is checking only the clauses most likely to matter.
        </p>
      </div>
    `);
  ...
  setBody(`
    ...
    ${
      risks.length
                ...
                (risk, index) => `
                  <article class="termscope-risk">
                    <div class="termscope-risk-top">
                      ...
                      <span
                        class="termscope-severity ${escapeHtml(
                          risk.severity ||
                          "medium"
                        )}"
                      >
                    ...
                    ${
                      risk.action
                        ? `
                          <p class="termscope-action">
                            What you can do: ${escapeHtml(
                              risk.action
                            )}
                          </p>
                        `
        ...
        `
        : `
          <div class="termscope-empty">
            <strong>
              No major problems found
            </strong>

            <p class="termscope-muted">
              The AI did not identify a serious issue in the selected categories. It can still miss context.
            </p>
          </div>
        `
    }
...
function renderError(message) {
  setBody(`
    <div class="termscope-error">
      <strong>
        TermScope could not finish the analysis.
      </strong>
    ...
    <button
      class="termscope-primary"
      id="termscope-open-settings"
      type="button"
    >
      Open settings
    </button>
  ...
  host
    ...
    .addEventListener(
      ...
      () => {
        analyzePolicies(
          lastSelectedPolicies.length
            ? lastSelectedPolicies
            : policies.slice(0, 1)
        );
      }
    );
...
function renderSettings() {
  ...
  setBody(`
    <div class="termscope-settings">
      ...
      <p class="termscope-muted">
        Choose what TermScope focuses on.
      </p>
      ...
      <label class="termscope-setting">
        <span>
          ...
          <small>
            Only beside Terms or Privacy links
          </small>
        </span>
      ...
      <div class="termscope-priorities">
        ${Object.entries(
          ...
          .map(
            ([key, label]) => `
              <label
                class="termscope-priority"
              >
                <input
                  type="checkbox"
                  value="${key}"
                  ${
                    settings.priorities.includes(
                      key
                    )
                      ? "checked"
                      : ""
      ...
      async () => {
        ...
        settings.priorities = [
          ...host.querySelectorAll(
            ".termscope-priority input:checked"
          )
        ].map(
          (input) => input.value
        );
